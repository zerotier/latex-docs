...
For clarity's sake, when I use the name zerotier in lowercase, I am specifically referring to the V1 ZeroTier protocol. When I use the name ZeroTier with capitalization, it is referring to the ZeroTier organization that maintains zerotier.
...
\begin{definition}[Root Servers]
	\emph{Root servers} are machines operated by ZeroTier that centrally handle the job of memorizing the mapping between addresses and identities, and the mapping between addresses and possible paths to contact the owner of that address. Within the protocol, they are responsible for public key distribution and for coordinating initial contact between newly connecting machines.
\end{definition}
