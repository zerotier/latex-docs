...
Nodes on a network can be given permission by the network controller to be ``multicast replicators''. Multicast replicators will replicate any multicast-frame packets they receive and forward them to every node they consider as a member of the multicast group specified by the packet. Nodes on a network will send their multicast frames only to a multicast replicator if one exists. If multiple exists it selects the one with the lowest path latency. If none exist the node will do sender-side replication to send their multicast frame to all members of the multicast group they know of.

MAC address 0xffffffffffff is the MAC address for the ``broadcast'' multicast group. When they join a network, all nodes automatically like this group with an ADI value of their assigned IPv4 address. This allows a multicast frame to be sent to all members of a network, or to only a member with a specific IPv4 address.
...
The only access control applied to a multicast group is whether or not a node has permission to join a network. In addition, multicast replicators replicate the plaintext of multicast-frames and re-encrypting each frame under only the peer-to-peer key of each's destination node. As such the plaintext of all multicast traffic over a network is effictively fully visible to all members of the network.
