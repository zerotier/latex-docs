...
We only implement the above protocol with those static choices of cryptographic primitives. \texttt{Noise\_XKpsk3} specifies we are implementing Noise XK with a preshared symmetric secret used on the third round of our handshake. \texttt{P384} specifies we are using eleptic curve P384 as our FIPS compliant Diffie-Helman (DH) primitive. \texttt{AESGCM} specifies we are using the AES block cipher in GCM mode as our symmetric primitive. \texttt{SHA384} is our FIPS compliant hash function. And finally, \texttt{hybridKyber1024} specifies we are adding hybrid encryption onto Noise using an ephemeral Kyber1024 DH handshake.
