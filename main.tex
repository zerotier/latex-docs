...
The ZeroTier V1 transport protocol does not provide identity hiding, forward secrecy nor replay protection. As such it is important for the security of both oneself and one's peers that one's private keys are not compromised. And if they are, all connections to peers authenticated with them ought to be immediately terminated and any permissions revoked. Replay protection can be obtained by using any protocol higher up on the protocol stack that implements packet deduplication, such as TCP\@. Identity hiding, forward secrecy and replay protection will all be security properties of the V2 ZeroTier transport protocol.
