...
To begin any connection through ZeroTier, the initiator node must send an unencrypted ``hello'' packet to the destination node. This hello includes the protocol version of the initiator, packet timestamp and the identity of the initiator. The destination node may decide to end the connection by simply not replying to this packet. If it does want to continue the connection and establish a secure session, it will perform Curve25519 key agreement between the initiator's identity and its own private key, generating a symmetric session key. It will then reply to the hello with a packet containing the same information about the destination node. The initiator will be able to use this to do key agreement with its private key and generate an identical session key. This session key is then used to encrypt all future packets between the two nodes. The encryption is performed using AES-GMAC-SIV with a 64-bit randomized IV\@. The Salsa20-Poly1305 stream cipher may also be used, in which case the 64-bit IV is used instead to mangle the session key per-packet.

Nodes will request to rekey their sessions every 60 seconds using the same protocol above. However the hello and hello reply packets are encrypted under the previous session key instead of plaintext. Rekeying requests will reset the heartbeat timer to prevent unnecessary traffic.
...
Every 14 seconds, for every known path, an ``echo'' packet is sent over it to the peer. Nodes are expected to reply to echo packets as soon as they are received over the same path they received it over. It is when the reply to the echo is received that the node can confirm that the path is still alive. If any packet is received over a specific path, the expiry timestamp on that path is updated to the current time. If the current time and the expiry timestamp on a path ever differ by over 243 seconds, the path is considered expired and is deleted from the table of known paths.
